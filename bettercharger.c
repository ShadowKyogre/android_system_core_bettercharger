...
/*
 * BetterCharger is fully based on CM Charger located on /system/core/charger
 * adding some good things to it for doing it more useful.
 * 
 * - Added support for draw_text_* on some relative positions (look for new functions).
 * - Modified draw_unknow, now show only error "?? %" if no images or no battery is founded
 *		do a little detailed error, and show the rest that is good.
 * - Added function currentDateTime(str, x, y) for easy print it, str is for strftime time values format.
 * - Added android_cyan and android_magenta and maybe some others.
 * - Added draw_menu() for easy print date and time on top.
 * - BOARD_SAMSUNG_DEVICE := true redefine BACKLIGHT_TOGGLE_PATH for Samsung path (almost i9300).
 * - FORCE_REBOOT_WHEN_FULL := true to force reboot the device when "fully" charged.
 * 		Added a reminder message on screen when enabled.
 * - BOARD_IMAGES_ON_SYSTEM := true no require a recovery that supports bettercharger
 * 		NOTE: Require this "mkdir -p /res/images".
 * - Added support for show battery charge_type and health.
 * 
 * ToDo
 * - Correct diferent time from Low Power Mode than OS.
 * - Reboot before alarm.
 * 
 */

//#define DEBUG_UEVENTS
#define BETTERCHARGER_KLOG_LEVEL 6
...
#define BATTERY_UNKNOWN_TIME    (5 * MSEC_PER_SEC)
...
/* For Samsung S3 GT-i9300 */
#define BACKLIGHT_TOGGLE_PATH "/sys/class/lcd/panel/device/backlight/panel/brightness"
#else
...
#if 1
#define LOGE(x...) do { KLOG_ERROR("bettercharger", x); } while (0)
#define LOGI(x...) do { KLOG_INFO("bettercharger", x); } while (0)
#define LOGV(x...) do { KLOG_DEBUG("bettercharger", x); } while (0)
#else
#define LOG_NDEBUG 0
#define LOG_TAG "bettercharger"
...
#ifdef IMAGES_ON_SYSTEM
/* For Samsung S3 GT-i9300 */
/* By default are located on /res/images, forced on recovery that not exists outside it
 * We want it on /system/bettercharger/images
 *  */
...
#else
/* By default are located on /res/images, forced on recovery */
static struct frame batt_anim_frames[] = {
    {
        .name = "bettercharger/battery_0",
        .disp_time = 750,
        .min_capacity = 0,
    },
    {
...
static void android_yellow(void)
{
    gr_color(0xa4, 0xc6, 0x39, 255);
}

static void android_cyan(void)
{
    gr_color(0x39, 0xa4, 0xc6, 255);
}

static void android_magenta(void)
{
    gr_color(0xc6, 0x39, 0xa4, 255);
}

/* Visit http://en.cppreference.com/w/cpp/chrono/c/strftime
 * for more information about date/time format */
static void currentDateTime(const char *str, int x, int y)
{
	time_t now = time(0);
	struct tm tstruct;
	char buf[15];
	tstruct = * localtime(&now);
	strftime(buf, sizeof(buf), str, &tstruct);
	draw_text(buf, x, y);
};

static void draw_text_line(const char *str, int line)
{
	draw_text(str, 0, char_height * line);
}

static void draw_text_centered(const char *str, int line)
{
	draw_text(str, -1, char_height * line);
}

static void draw_text_top_centered(const char *str)
{
	draw_text(str, -1, char_height);
}

static void draw_text_bottom_centered(const char *str)
{
	draw_text(str, -1, gr_fb_height() - char_height);
}

static void draw_text_top_left(const char *str)
{
	draw_text(str, 0, char_height);
}

static void draw_text_top_right(const char *str)
{
	draw_text(str, gr_fb_width()-gr_measure(str), char_height);
}

static void draw_text_bottom_left(const char *str)
{
	draw_text(str, 0, gr_fb_height() - char_height);
}

static void draw_text_bottom_right(const char *str)
{
	draw_text(str, gr_fb_width()-gr_measure(str), gr_fb_height() - char_height);
}

static void draw_text_bottom_warning(const char *str)
{
	draw_text(str, gr_fb_width()-gr_measure(str), gr_fb_height() - char_height*2);
}

#ifdef FORCE_REBOOT_WHEN_FULL
static bool is_fully_charged(struct charger *charger)
{
	if ( charger->batt_anim->capacity >= 100 ) {
		return false;
	} else {
		return true;
	}
...
#ifdef FORCE_REBOOT_WHEN_FULL
static void advice_reboot_when_full(void)
{
	android_yellow();
	draw_text_bottom_centered("AutoStart when charged!");
}
#endif
...
{
	android_cyan();
	/* On first line and centered */
	draw_text_top_centered("BetterCharger");
	/* Data & Time */
	android_magenta();
	currentDateTime("%d-%m-%Y", 0, char_height);
	currentDateTime("%H:%M", (gr_fb_width()-gr_measure("%H:%M")), char_height);
#ifdef FORCE_REBOOT_WHEN_FULL
	advice_reboot_when_full();
#endif
}
...
static void draw_health(void)
{
	...
	fd = open("/sys/class/power_supply/battery/health", O_RDONLY);
	if(fd < 0) {
		LOGE("Failed to open health node, aborting");
	} else {
		read(fd, cap_str_tmp, sizeof(cap_str_tmp));
		close(fd);
		
		if ( strncmp(cap_str_tmp,"N/A",3) == 0 ) {
			strcpy(cap_str, "Unknow battery health");
		} else {
			strcpy(cap_str, cap_str_tmp);
			strcat(cap_str, "battery health");
		}
		
		str_len_px = gr_measure(cap_str);
		x = (gr_fb_width() - str_len_px) / 2;
		y = (gr_fb_height() / 2 )+ char_height * 17;
		android_cyan();
		gr_text(x, y, cap_str, 0);
	}
}

static void draw_charge_type(void)
{
	int fd = -1;
	char cap_str[64];
	char cap_str_tmp[64];
	...
	fd = open("/sys/class/power_supply/battery/charge_type", O_RDONLY);
	if(fd < 0) {
		LOGE("Failed to open charge_type node, aborting");
	} else {
		read(fd, cap_str_tmp, sizeof(cap_str_tmp));
		close(fd);
		
		if ( strncmp(cap_str_tmp,"N/A",3) == 0 ) {
			strcpy(cap_str, "Battery not charging");
		} else {
			strcpy(cap_str, cap_str_tmp);
			strcat(cap_str, "charge");
		}
		
		str_len_px = gr_measure(cap_str);
		x = (gr_fb_width() - str_len_px) / 2;
		y = (gr_fb_height() / 2 )+ char_height * 15;
		android_cyan();
		gr_text(x, y, cap_str, 0);
	}
}
...
static void draw_unknown(struct charger *charger)
{
    ...
    if (charger->surf_unknown) {
		...
		if(fd < 0) {
			...
			read(fd, cap_status_str, sizeof(cap_status_str));
			close(fd);
		}
		
		y = draw_text(cap_status_str, -1, -1);
...
#ifdef FORCE_REBOOT_WHEN_FULL
	while ( is_fully_charged(charger) ) {
#else
    while (true) {
#endif
        int64_t now = curr_time_ms();
...
int main(int argc, char **argv)
{
    ...
    klog_init();
    klog_set_level(BETTERCHARGER_KLOG_LEVEL);
...
#ifdef IMAGES_ON_SYSTEM
/* For Samsung S3 GT-i9300 */
	ret = res_create_display_surface("../../system/bettercharger/images/battery_fail", &charger->surf_unknown);
#else
	ret = res_create_display_surface("bettercharger/battery_fail", &charger->surf_unknown);
#endif
